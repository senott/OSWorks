...
public class ServiceOrderShowService {
    ...
    public ResponseEntity<ServiceOrderDTO> show(Long id) {
        Optional<ServiceOrder> serviceOrder = serviceOrderRepository.findById(id);

        return serviceOrder.map(order -> ResponseEntity.ok(serviceOrderToDTOService.toDTO(order))).orElseGet(() -> ResponseEntity.notFound().build());
    }
}
